...
# Moves smaller than this are "flat" — graded neither correct nor wrong.
# Nifty tick noise / bid-ask bounce is ±3-5 pts; counting a 2-pt move as a
# directional win or loss corrupts accuracy stats with pure noise.
MIN_MOVE_PTS = 5.0
...
def clear_all_predictions() -> int:
    """Delete the entire prediction log (e.g. after fixing a data anchor bug)."""
    if LIVE_LOG_FILE.exists():
        n = sum(1 for _ in open(LIVE_LOG_FILE))
        LIVE_LOG_FILE.unlink()
        return n
    return 0


def clear_predictions_for_date(target_date: str) -> int:
    """Remove only predictions logged on a specific date; keep the rest."""
...
def verify_due_predictions(current_price: float) -> list:
    ...
    with open(LIVE_LOG_FILE) as f:
        for line in f:
            ...
            # (actual_price is the verification marker; "flat" records keep correct=None)
            if r.get("actual_price") is None and r.get("target_ts"):
                try:
                    target_dt = datetime.fromisoformat(r["target_ts"])
                    if now >= target_dt:
                        entry = r["entry_price"]
                        r["actual_price"] = round(current_price, 2)
                        move_pts = current_price - entry
                        if abs(move_pts) < MIN_MOVE_PTS:
                            # Flat move — inside the noise band. Grade as neither
                            # correct nor wrong so coin-flip noise doesn't pollute stats.
                            r["correct"] = None
                            r["flat"]    = True
                        else:
                            r["correct"] = int(
                                (r["direction"] == 1 and move_pts > 0) or
                                (r["direction"] == 0 and move_pts < 0)
                            )
                            r["flat"] = False
                        ...
                        # Direction can be right while the predicted price is
                        # far off. Track the price error too.
                        tgt = r.get("target_price", 0)
                        if tgt and tgt > 0:
                            r["price_error_pts"] = round(abs(current_price - tgt), 1)
                            r["price_error_pct"] = round(abs(current_price - tgt) / current_price * 100, 3)
                            # Predicted move vs actual move (how well magnitude matched)
                            pred_move = tgt - entry
                            actual_move = current_price - entry
                            r["pred_move_pts"]   = round(pred_move, 1)
                            r["actual_move_pts"] = round(actual_move, 1)
                            # Magnitude hit: did actual move reach at least 50% of predicted move?
                            if abs(pred_move) > 0.1:
                                r["magnitude_ratio"] = round(actual_move / pred_move, 2)
                            else:
                                r["magnitude_ratio"] = 0.0
                        newly_verified.append(r)
                except Exception:
                    pass
            rows.append(r)

    # Rewrite log with verifications (flat ones are verified too, just ungraded)
    if newly_verified:
        with open(LIVE_LOG_FILE, "w") as f:
            for r in rows:
...
def get_daily_history(target_date: str = None) -> pd.DataFrame:
    ...
    # Flat = verified but move was inside the ±5pt noise band
    if "flat" in df.columns:
        df.loc[(df["status"].isna()) & (df["flat"] == True), "status"] = "➖ Flat (<5 pts)"
...
def get_calibration_summary(target_date: str = None) -> dict:
    ...
    # ── Magnitude accuracy (price error) ──────────────────────────────────
    magnitude = {}
    if "price_error_pts" in verified.columns:
        with_price = verified[verified["price_error_pts"].notna()]
        if len(with_price) > 0:
            for hz in with_price["horizon"].unique():
                sub = with_price[with_price["horizon"] == hz]
                magnitude[hz] = {
                    "n":            len(sub),
                    "mae_pts":      round(float(pd.to_numeric(sub["price_error_pts"], errors="coerce").mean()), 1),
                    "mae_pct":      round(float(pd.to_numeric(sub["price_error_pct"], errors="coerce").mean()), 3),
                }
